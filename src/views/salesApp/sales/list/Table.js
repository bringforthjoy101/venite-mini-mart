...
const TransactionTable = () => {
	...
	const downloadPDF = () => {
		...
		doc.autoTable({
			...
			head: [['SaleID', 'Amount', 'Amount Paid', 'Status', 'Mode', 'Date', 'Initiator']],
		})
			...
			doc.autoTable({
				...
				body: [
					[
						...
						arr.amountPaid?.toLocaleString('en-US', { style: 'currency', currency: 'NGN' }),
						arr.status,
						arr.mode,
						arr.createdAt,
						arr.user.fullName,
					...
					<Col xl="6" sm="12" className="d-flex align-items-sm-center justify-content-lg-end justify-content-center pr-lg-3 p-0 mt-lg-0 mt-1">
						<UncontrolledButtonDropdown>
							...
							<DropdownMenu right>
								...
								<DropdownItem className="w-100" onClick={() => printSale(filteredData)}>
									<Printer size={15} />
